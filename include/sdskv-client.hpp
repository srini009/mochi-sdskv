...
inline size_t object_size(const std::string& s) { return s.size(); }
...
inline void* object_data(std::string& s) { return (void*)s.data(); }
...
class client {
  ...
  public:
     ...
     */
    client(margo_instance_id mid) : m_mid(mid)
    {
     ...
     * @param transfer_ownership Whether this client instance should take
     * ownership and free the underlying handle in its destructor.
     */
    client(margo_instance_id mid,
           sdskv_client_t    c,
           bool              transfer_ownership = false)
        : m_mid(mid), m_client(c), m_owns_client(transfer_ownership)
    {}
     ...
     */
    client(client&& c)
        : m_mid(c.m_mid), m_client(c.m_client), m_owns_client(c.m_owns_client)
    {
    ...
    /**
     * @brief Move assignment operator, will invalidate the client that is moved
     * from.
     */
    client& operator=(client&& c)
    {
        if (this == &c) return *this;
        if (m_client && m_owns_client) { sdskv_client_finalize(m_client); }
        m_mid           = c.m_mid;
        m_client        = c.m_client;
    ...
    /**
     * @brief Destructor. If this instance owns the underlying C handle, this
     * handle will be destroyed.
     */
    ~client()
    {
        if (m_owns_client && m_client) sdskv_client_finalize(m_client);
    }

    /**
     * @brief Cast operator to sdskv_client_t.
     */
    operator sdskv_client_t() const { return m_client; }
     ...
     */
    operator bool() const { return m_client != SDSKV_CLIENT_NULL; }
    ...
    inline void put(const database& db, const K& key, const V& value) const
    {
        put(db, object_data(key), object_size(key), object_data(value),
            object_size(value));
    }
    ...
    inline void put_multi(const database&                 db,
                          ...
                          const std::vector<hg_size_t>&   vsizes) const
    {
        if (keys.size() != ksizes.size() || keys.size() != values.size()
            || keys.size() != vsizes.size()) {
            throw std::length_error(
                "Provided vectors should have the same size");
        }
        put_multi(db, keys.size(), keys.data(), ksizes.data(), values.data(),
                  vsizes.data());
    }

    /**
     * @brief Templated put method. Meant to work with std::vector<X> and
     * std::string types. X must be a standard layout  type.
     *
     * @tparam K std::vector<char> or std::string.
     * @tparam V std::vector<char> or std::string.
     * @param db Database instance.
                          ...
                          const std::vector<V>& values)
    {
        ...
        std::vector<const void*> kdata;
        kdata.reserve(keys.size());
        std::vector<const void*> vdata;
        vdata.reserve(values.size());
        std::vector<hg_size_t> ksizes;
        ksizes.reserve(keys.size());
        std::vector<hg_size_t> vsizes;
        vsizes.reserve(values.size());
     ...
     * meant to work with keys and values of type std::vector<X> and
     * std::string. X must be a standard layout type.
     *
                          ...
                          const IV&       vend) const
    {
        ...
        if (count != std::distance(vbegin, vend)) {
            throw std::length_error(
                "Provided iterators should point to the same number of "
                "objects");
        }
        std::vector<const void*> kdata;
        kdata.reserve(count);
    ...
    inline void put_packed(const database&               db,
                           const std::string&            packed_keys,
                           const std::vector<hg_size_t>& ksizes,
                           const std::string&            packed_values,
                           const std::vector<hg_size_t>& vsizes) const
    {
        put_packed(db, ksizes.size(), packed_keys.data(), ksizes.data(),
                   packed_values.data(), vsizes.data());
    }
    ...
    inline void put_packed(const database&    db,
                           ...
                           hg_size_t          packed_data_size) const;
     ...
     */
    std::vector<bool> exists_multi(const database&    db,
                                   size_t             num,
                                   const void* const* key,
                                   const hg_size_t*   ksize) const;
     ...
     */
    template <typename K>
    inline std::vector<bool> exists_multi(const database&       db,
     ...
     */
    hg_size_t
    length(const database& db, const void* key, hg_size_t ksize) const;
     ...
     */
    template <typename K>
    inline hg_size_t length(const database& db, const K& key) const
    {
                             ...
                             std::vector<hg_size_t>& vsizes) const
    {
        ...
        return length_multi(db, keys.size(), kdata.data(), ksizes.data(),
                            vsizes.data());
    }
     ...
     */
    template <typename K>
    inline std::vector<hg_size_t> length_multi(const database&       db,
                                               const std::vector<K>& keys) const
    {
                             ...
                             std::vector<hg_size_t>&       vsizes) const
    {
        ...
        return length_packed(db, ksizes.size(), keys.data(), ksizes.data(),
                             vsizes.data());
    }
    ...
    /**
     * @brief Equivalent of sdskv_get. Will throw an exception if the key
     * doesn't exist, or if the buffer allocated for the value is too small.
     *
     * @param db Database instance.
     * @param key Key.
     * @param ksize Size of the key.
    ...
    /**
     * @brief Templated version of get, meant to be used with std::vector<X> and
     * std::string. X must be a standard layout type.
     *
     * @tparam K Key type.
     * @tparam V Value type.
     ...
     */
    template <typename K, typename V>
    inline bool get(const database& db, const K& key, V& value) const
    {
        ...
        try {
            ...
            if (ex.error() == SDSKV_ERR_SIZE) {
                object_resize(value, s);
                get(db, object_data(key), object_size(key), object_data(value),
                    &s);
            } else {
                throw;
            }
     ...
     */
    template <typename K, typename V>
    inline V get(const database& db, const K& key) const
    {
        V value;
        get(db, key, value);
        return value;
                          ...
                          std::vector<hg_size_t>&         vsizes) const
    {
        ...
        return get_multi(db, keys.size(), keys.data(), ksizes.data(),
                         values.data(), vsizes.data());
    }
     ...
     */
    template <typename K, typename V>
    inline std::vector<V> get_multi(const database&       db,
                                    const std::vector<K>& keys)
    {
                           ...
                           std::vector<hg_size_t>&       vsizes) const
    {
        ...
        vsizes.resize(count);
        bool b = get_packed(
            db, &count, packed_keys.data(), ksizes.data(), packed_values.size(),
            const_cast<char*>(packed_values.data()), vsizes.data());
        vsizes.resize(count);
        return b;
    }
     ...
     * @param ksize Size of the key.
     */
    void erase(const database& db, const void* key, hg_size_t ksize) const;
     ...
     */
    template <typename K>
    inline void erase(const database& db, const K& key) const
    {
    ...
    template <typename K>
    inline void
    list_keys(const database& db, const K& start_key, std::vector<K>& keys)
    {
        hg_size_t max_keys = keys.size();
        if (max_keys == 0) return;
        std::vector<void*> kdata;
        kdata.reserve(keys.size());
        std::vector<hg_size_t> ksizes;
        ksizes.reserve(keys.size());
        for (auto& k : keys) {
                          ...
                          std::vector<K>& keys)
    {
        ...
        try {
            list_keys(db, object_data(start_key), object_size(start_key),
                      object_data(prefix), object_size(prefix), kdata.data(),
                      ksizes.data(), &max_keys);
        } catch (exception& e) {
    ...
    inline void list_keyvals(const database& db,
                             ...
                             hg_size_t*      max_items) const
    {
        list_keyvals(db, start_key, start_ksize, NULL, 0, keys, ksizes, values,
                     vsizes, max_items);
    }
                             ...
                             std::vector<V>& values) const
    {
        ...
        try {
            list_keyvals(db, object_data(start_key), object_size(start_key),
                         kdata.data(), ksizes.data(), vdata.data(),
                         vsizes.data(), &max_keys);
        } catch (exception& e) {
                             ...
                             std::vector<V>& values) const
    {
        ...
        try {
            list_keyvals(db, object_data(start_key), object_size(start_key),
                         object_data(prefix), object_size(prefix), kdata.data(),
                         ksizes.data(), vdata.data(), vsizes.data(), &max_keys);
        } catch (exception& e) {
            if (e.error() == SDSKV_ERR_SIZE) {
                for (unsigned i = 0; i < max_keys; i++) {
                    object_resize(keys[i], ksizes[i]);
    ...
    /**
     * @brief Migrates a given set of keys from a source to a destination
     * database.
     *
     ...
     */
    void migrate(const database&    source_db,
                 const database&    dest_db,
                 hg_size_t          num_items,
                 const void* const* keys,
     ...
     * @param flag SDSKV_KEEP_ORIGINAL or SDSKV_REMOVE_ORIGINAL.
     */
    inline void migrate(const database&                source_db,
                        const database&                dest_db,
                        const std::vector<const void*> keys,
                        const std::vector<hg_size_t>   ksizes,
                        int flag = SDSKV_KEEP_ORIGINAL) const
    {
        ...
        migrate(source_db, dest_db, keys.size(), keys.data(), ksizes.data(),
                flag);
    }
                        ...
                        int                  flag = SDSKV_KEEP_ORIGINAL) const
    {
        ...
        migrate(source_db, dest_db, kdata, ksizes, flag);
    }
    ...
    void migrate(const database& source_db,
                 const database& dest_db,
                 const void*     key_range[2],
                 const hg_size_t key_sizes[2],
                 int             flag = SDSKV_KEEP_ORIGINAL) const;
    ...
    inline void migrate(const database&        source_db,
                        ...
                        int                    flag = SDSKV_KEEP_ORIGINAL) const
    {
        const void* key_range_arr[2]
            = {object_data(key_range.first), object_data(key_range.second)};
        const hg_size_t key_sizes_arr[2]
            = {object_size(key_range.first), object_size(key_range.second)};
    ...
    inline void migrate(const database& source_db,
                        ...
                        int             flag = SDSKV_KEEP_ORIGINAL) const
    {
        migrate(source_db, dest_db, object_data(prefix), object_size(prefix),
                flag);
    }
     ...
     *
     * @param source_db Source database (will be modified to point to the new
     * provider).
...
class provider_handle {
  ...
  public:
     ...
     */
    provider_handle(client& c, hg_addr_t addr, uint16_t provider_id = 0)
        : m_client(&c)
    {
        int ret = sdskv_provider_handle_create(c.m_client, addr, provider_id,
                                               &m_ph);
        _CHECK_RET(ret);
    }
    ...
    provider_handle(const provider_handle& other)
        : m_client(other.m_client), m_ph(other.m_ph)
    {
     ...
     */
    operator sdskv_provider_handle_t() const { return m_ph; }
};
...
class database {
  ...
  public:
     ...
     */
    operator sdskv_database_id_t() const { return m_db_id; }

    /**
     * @brief @see client::put.
     */
    template <typename... T> void put(T&&... args) const
    {
     ...
     */
    template <typename... T> void put_multi(T&&... args) const
    {
     ...
     */
    template <typename... T> void put_packed(T&&... args) const
    {
     ...
     */
    template <typename... T> decltype(auto) length(T&&... args) const
    {
     ...
     */
    template <typename... T> decltype(auto) length_multi(T&&... args) const
    {
     ...
     */
    template <typename... T> decltype(auto) length_packed(T&&... args) const
    {
     ...
     */
    template <typename... T> decltype(auto) get(T&&... args) const
    {
     ...
     */
    template <typename... T> decltype(auto) get_multi(T&&... args) const
    {
     ...
     */
    template <typename... T> decltype(auto) get_packed(T&&... args) const
    {
     ...
     */
    template <typename... T> decltype(auto) exists(T&&... args) const
    {
     ...
     */
    template <typename... T> decltype(auto) exists_multi(T&&... args) const
    {
     ...
     */
    template <typename... T> void erase(T&&... args) const
    {
     ...
     */
    template <typename... T> void erase_multi(T&&... args) const
    {
     ...
     */
    template <typename... T> decltype(auto) list_keys(T&&... args) const
    {
     ...
     */
    template <typename... T> decltype(auto) list_keyvals(T&&... args) const
    {
     ...
     */
    template <typename... T>
    void migrate(const std::string& dest_provider_addr,
                 uint16_t           dest_provider_id,
                 T&&... args)
    {
        m_ph.m_client->migrate(*this, dest_provider_addr, dest_provider_id,
                               std::forward<T>(args)...);
    }
};

inline database client::open(const provider_handle& ph,
...
inline void client::put(const database& db,
                        ...
                        hg_size_t       vsize) const
{
    int ret = sdskv_put(db.m_ph.m_ph, db.m_db_id, key, ksize, value, vsize);
    _CHECK_RET(ret);
}
...
inline void client::put_multi(const database&    db,
                              ...
                              const hg_size_t*   vsizes) const
{
    int ret = sdskv_put_multi(db.m_ph.m_ph, db.m_db_id, count, keys, ksizes,
                              values, vsizes);
    _CHECK_RET(ret);
}
...
inline void client::put_packed(const database&  db,
                               ...
                               const hg_size_t* vsizes) const
{
    int ret = sdskv_put_packed(db.m_ph.m_ph, db.m_db_id, count, keys, ksizes,
                               values, vsizes);
    _CHECK_RET(ret);
}
...
inline void client::put_packed(const database&    db,
                               ...
                               hg_size_t          bulk_handle_size) const
{
    int ret
        = sdskv_proxy_put_packed(db.m_ph.m_ph, db.m_db_id, origin_addr.c_str(),
                                 count, bulk_handle, bulk_handle_size);
    _CHECK_RET(ret);
}

inline hg_size_t
client::length(const database& db, const void* key, hg_size_t ksize) const
{
    hg_size_t vsize;
    int       ret = sdskv_length(db.m_ph.m_ph, db.m_db_id, key, ksize, &vsize);
    _CHECK_RET(ret);
    return vsize;
}

inline bool
client::exists(const database& db, const void* key, hg_size_t ksize) const
{
    int flag;
...
inline std::vector<bool> client::exists_multi(const database&    db,
                                              size_t             num,
                                              const void* const* keys,
                                              const hg_size_t*   ksizes) const
{
    ...
    int ret = sdskv_exists_multi(db.m_ph.m_ph, db.m_db_id, num, keys, ksizes,
                                 flags.data());
    _CHECK_RET(ret);
...
inline bool client::length_multi(const database&    db,
                                 ...
                                 hg_size_t*         vsizes) const
{
    int ret = sdskv_length_multi(db.m_ph.m_ph, db.m_db_id, num, keys, ksizes,
                                 vsizes);
    _CHECK_RET(ret);
    return true;
}
...
inline bool client::length_packed(const database&  db,
                                  ...
                                  hg_size_t*       vsizes) const
{
    int ret = sdskv_length_packed(db.m_ph.m_ph, db.m_db_id, num, keys, ksizes,
                                  vsizes);
    _CHECK_RET(ret);
    return true;
}
...
inline bool client::get(const database& db,
                        ...
                        hg_size_t*      vsize) const
{
    int ret = sdskv_get(db.m_ph.m_ph, db.m_db_id, key, ksize, value, vsize);
    _CHECK_RET(ret);
    return true;
}
...
inline bool client::get_multi(const database&    db,
                              ...
                              hg_size_t*         vsizes) const
{
    int ret = sdskv_get_multi(db.m_ph.m_ph, db.m_db_id, count, keys, ksizes,
                              values, vsizes);
    _CHECK_RET(ret);
    return true;
}
...
inline bool client::get_packed(const database&  db,
                               ...
                               hg_size_t*       vsizes) const
{
    int ret = sdskv_get_packed(db.m_ph.m_ph, db.m_db_id, count, keys, ksizes,
                               valbufsize, values, vsizes);
    _CHECK_RET(ret);
    return true;
}

inline void
client::erase(const database& db, const void* key, hg_size_t ksize) const
{
    int ret = sdskv_erase(db.m_ph.m_ph, db.m_db_id, key, ksize);
    _CHECK_RET(ret);
}
...
inline void client::erase_multi(const database&    db,
                                ...
                                const hg_size_t*   ksizes) const
{
    int ret = sdskv_erase_multi(db.m_ph.m_ph, db.m_db_id, num, keys, ksizes);
    _CHECK_RET(ret);
}
...
inline void client::list_keys(const database& db,
                              ...
                              hg_size_t*      max_keys) const
{
    int ret = sdskv_list_keys_with_prefix(db.m_ph.m_ph, db.m_db_id, start_key,
                                          start_ksize, prefix, prefix_size,
...
inline void client::list_keyvals(const database& db,
                                 ...
                                 hg_size_t*      max_items) const
{
    int ret = sdskv_list_keyvals_with_prefix(
        db.m_ph.m_ph, db.m_db_id, start_key, start_ksize, prefix, prefix_size,
        keys, ksizes, values, vsizes, max_items);
    _CHECK_RET(ret);
}

inline void client::migrate(const database&    source_db,
                            const database&    dest_db,
                            hg_size_t          num_items,
                            const void* const* keys,
                            const hg_size_t*   key_sizes,
                            int                flag) const
{
    ...
    sdskv_provider_handle_get_info(dest_db.m_ph.m_ph, NULL, &addr,
                                   &target_pr_id);
    ...
    hg_return_t       hret
        = margo_addr_to_string(mid, target_addr_str.data(), &addr_size, addr);

    if (hret != 0)
        throw std::runtime_error(
            "margo_addr_to_string failed in client::migrate");

    int ret = sdskv_migrate_keys(
        source_db.m_ph.m_ph, source_db.m_db_id, target_addr_str.data(),
        target_pr_id, dest_db.m_db_id, num_items, keys, key_sizes, flag);
    _CHECK_RET(ret);
}
                            ...
                            int             flag) const
{
    ...
    int ret = sdskv_migrate_key_range(
        source_db.m_ph.m_ph, source_db.m_db_id, target_addr_str.data(),
        target_pr_id, dest_db.m_db_id, key_range, key_sizes, flag);
    _CHECK_RET(ret);
}
                            ...
                            int             flag) const
{
    ...
    int ret = sdskv_migrate_keys_prefixed(
        source_db.m_ph.m_ph, source_db.m_db_id, target_addr_str.data(),
        target_pr_id, dest_db.m_db_id, prefix, prefix_size, flag);
    _CHECK_RET(ret);
}
                            ...
                            int                flag) const
{
    ...
    hg_return_t       hret
        = margo_addr_lookup(mid, dest_provider_addr.c_str(), &addr);
    if (hret != HG_SUCCESS)
        throw std::runtime_error("margo_addr_lookup failed in client::migrate");
    ...
    int ret = sdskv_migrate_database(db.m_ph.m_ph, db.m_db_id,
                                     ...
                                     dest_provider_id, dest_root.c_str(), flag);
    _CHECK_RET(ret);
