...
class AbstractBenchmark {
    ...
    using benchmark_factory_function
        = std::function<std::unique_ptr<AbstractBenchmark>(
            Json::Value&, MPI_Comm, RemoteDatabase&)>;
...
class AbstractAccessBenchmark : public AbstractBenchmark {
  ...
  public:
        ...
        : AbstractBenchmark(std::forward<T>(args)...)
    {
        ...
        } else if (config["key-sizes"].isArray()
                   && config["key-sizes"].size() == 2) {
        ...
        } else if (config["val-sizes"].isArray()
                   && config["val-sizes"].size() == 2) {
  ...
  public:
    ...
    virtual void setup() override
    {
        ...
        for (unsigned i = 0; i < m_num_entries; i++) {
            size_t ksize
                = m_key_size_range.first
                + (rand() % (m_key_size_range.second - m_key_size_range.first));
            m_keys.push_back(gen_random_string(ksize));
            size_t vsize
                = m_val_size_range.first
                + (rand() % (m_val_size_range.second - m_val_size_range.first));
...
class PutMultiBenchmark : public PutBenchmark {
  ...
  public:
    ...
    virtual void teardown() override
    {
        ...
        m_ksizes.resize(0);
        m_ksizes.shrink_to_fit();
        m_kptrs.resize(0);
        m_kptrs.shrink_to_fit();
        m_vsizes.resize(0);
        m_vsizes.shrink_to_fit();
        m_vptrs.resize(0);
        m_vptrs.shrink_to_fit();
    }
};
...
class GetBenchmark : public AbstractAccessBenchmark {
  ...
  public:
    ...
    virtual void setup() override
    {
        ...
        if (m_reuse_buffer)
            m_vals_buffer.resize(1,
                                 std::string(m_val_size_range.second - 1, 0));
        else
            m_vals_buffer.resize(m_num_entries,
                                 std::string(m_val_size_range.second - 1, 0));
    }
    ...
    virtual void execute() override
    {
        ...
        for (unsigned i = 0; i < m_num_entries; i++) {
            ...
            db.get((const void*)key.data(), key.size(), (void*)val.data(),
                   &vsize);
            if (!m_reuse_buffer) j += 1;
        }
    }
  ...
  public:
    ...
    virtual void setup() override
    {
        ...
        if (m_vals_buffer.size() != m_batch_size && m_reuse_buffer) {
            m_vals_buffer.resize(m_batch_size,
                                 std::string(m_val_size_range.second - 1, 0));
        }
    }
...
/**
 * LengthBenchmark executes a series of LENGTH operations and measures their
 * duration.
 */
class LengthBenchmark : public GetBenchmark {
  ...
  public:
    template <typename... T>
    LengthBenchmark(T&&... args) : GetBenchmark(std::forward<T>(args)...)
    {}
  ...
  public:
    ...
    virtual void execute() override
    {
        ...
        while (remaining != 0) {
            ...
            db.length_multi(count, m_kptrs.data(), m_ksizes.data(),
                            m_vsizes.data() + k);
            remaining -= count;
            j += count;
            if (!m_reuse_buffer) k += count;
...
class EraseBenchmark : public GetBenchmark {
  ...
  public:
    template <typename... T>
    EraseBenchmark(T&&... args) : GetBenchmark(std::forward<T>(args)...)
    {}
...
/**
 * ListKeysBenchmark executes a series of LIST KEYS operations and measures
 * their duration.
 */
class ListKeysBenchmark : public GetBenchmark {
  ...
  public:
    ...
    virtual void setup() override
    {
        GetBenchmark::setup();
        if (m_reuse_buffer)
            m_keys_buffer.resize(m_batch_size,
                                 std::string(m_key_size_range.second - 1, 0));
        else
            m_keys_buffer.resize(m_num_entries,
                                 std::string(m_key_size_range.second - 1, 0));
        m_ksizes.resize(m_batch_size);
        m_kptrs.resize(m_batch_size);
    }
    ...
    virtual void execute() override
    {
        ...
        while (remaining != 0) {
            ...
            db.list_keys(start_key.data(), start_key.size(),
                         ...
                         &count);
            if (count > 0) {
                start_key = std::string((const char*)m_kptrs[count - 1],
                                        m_ksizes[count - 1]);
            }
            if (!m_reuse_buffer) k += count;
            remaining -= count;
    ...
    {
        GetBenchmark::teardown();
        m_keys_buffer.resize(0);
        m_keys_buffer.shrink_to_fit();
        m_ksizes.resize(0);
        m_ksizes.shrink_to_fit();
        m_kptrs.resize(0);
        m_kptrs.shrink_to_fit();
...
/**
 * ListKeysBenchmark executes a series of LIST KEYVALS operations and measures
 * their duration.
 */
  ...
  public:
    ...
    virtual void setup() override
    {
        ...
        if (m_reuse_buffer)
            m_vals_buffer.resize(m_batch_size,
                                 std::string(m_val_size_range.second - 1, 0));
        // if !m_reuse_buffer the m_vals_buffer vector already has the right
        // size
        m_vsizes.resize(m_batch_size);
        m_vptrs.resize(m_batch_size);
    }
...
int main(int argc, char** argv)
{
    ...
    if (!config_file.good() && rank == 0) {
        std::cerr << "Could not read configuration file " << argv[1]
                  << std::endl;
        MPI_Abort(MPI_COMM_WORLD, -1);
    }
...
static void run_server(MPI_Comm comm, Json::Value& config)
{
    ...
    mid = margo_init(protocol.c_str(), MARGO_SERVER_MODE, use_progress_thread,
                     rpc_thread_count);
    ...
    sdskv_db_type_t db_type
        = database_type_from_string(database_config["type"].asString());
    sdskv_config_t db_config = {.db_name         = db_name.c_str(),
...
static void run_client(MPI_Comm comm, Json::Value& config)
{
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    {
        ...
        if (config["benchmarks"].isArray()) {
            for (auto& bench_config : config["benchmarks"]) {
                ...
                benchmarks.push_back(
                    AbstractBenchmark::create(type, bench_config, comm, db));
        ...
        for (unsigned i = 0; i < benchmarks.size(); i++) {
            ...
            if (num_clients != 1) {
                MPI_Gather(local_timings.data(), local_timings.size(),
                           MPI_DOUBLE, global_timings.data(),
                           local_timings.size(), MPI_DOUBLE, 0, comm);
            } else {
            ...
            if (rank == 0) {
                ...
                std::cout << "-----------------"
                          << std::string(types[i].size(), '-')
                          << "-----------------" << std::endl;
                double average = std::accumulate(global_timings.begin(),
                                                 global_timings.end(), 0.0)
                               / n;
                double variance = std::accumulate(
                    global_timings.begin(), global_timings.end(), 0.0,
                ...
                double median
                    = (n % 2)
                        ? global_timings[n / 2]
                        : ((global_timings[n / 2] + global_timings[n / 2 - 1])
                           / 2.0);
        ...
        if (rank == 0) margo_shutdown_remote_instance(mid, server_addr);
    }
    margo_addr_free(mid, server_addr);
    margo_finalize(mid);
}
    ...
    // initialize and start client
    {
        ...
        if (config["benchmarks"].isArray()) {
            for (auto& bench_config : config["benchmarks"]) {
                ...
                benchmarks.push_back(AbstractBenchmark::create(
                    type, bench_config, MPI_COMM_WORLD, db));
                repetitions.push_back(bench_config["repetitions"].asUInt());
            }
        } else {
            auto&       bench_config = config["benchmarks"];
            std::string type         = bench_config["type"].asString();
            types.push_back(type);
            benchmarks.push_back(AbstractBenchmark::create(type, bench_config,
                                                           MPI_COMM_WORLD, db));
            repetitions.push_back(bench_config["repetitions"].asUInt());
        }
        // main execution loop
        for (unsigned i = 0; i < benchmarks.size(); i++) {
            ...
            double average = std::accumulate(global_timings.begin(),
                           ...
                           / n;
            double variance
                = std::accumulate(global_timings.begin(), global_timings.end(),
                                  0.0, [average](double acc, double x) {
                                      return acc + std::pow((x - average), 2);
                                  });
            variance /= n;
            double stddev = std::sqrt(variance);
